...
{
  /**
   * Background color for basic text.
   * TODO @revise move these to some global properties
   */
  ...
  /**
   * Draw a tinted version of the map Element
  ...
  {         
      ...
      // TODO @revise we're creating a new image buffer
      ...
      // Filling background with required color
      bg.setColor(useCol);
      bg.fillRect(0, 0, dw, dh);
      
      // Drawing the element regularly
      drawScaled(bg, 0, 0, scale);
      
      // Drawing a transparent color film over the drawn element
      bg.setComposite(UtilityFunctions.getGhostlyComposite());        
      bg.fillRect(0, 0, dw, dh);
      bg.dispose();

      // TODO @revise creating yet another buffer, setting an opacity or something to what we just did
      bi = UtilityFunctions.makeColorTransparent(bi, useCol);

      // Drawing the result on the canvas
   ...
   */
  private void drawStringToCanvas(Graphics2D g, boolean bForceTextInBounds, Color backgroundColor,
      boolean drawAttributes, GametableCanvas canvas)
  {
   ...
   // Add some padding to width and height
      ...
      final Rectangle backgroundRect = new Rectangle(
      	pogDrawCoords.x + (viewWidth - totalWidth) / 2,
      	pogDrawCoords.y - totalHeight - 4,
      	totalWidth,
      	totalHeight);
