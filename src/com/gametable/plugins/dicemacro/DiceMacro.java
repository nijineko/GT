...
public class DiceMacro
{
	...
	private static class TermList
	{
		...
		/*
		 * public Term get(final int i) { return m_terms.get(i); }
		 */
	...
	private class Term
	{
		...
		/**
		 * The number of highest rolls to keep. If negative, keeps the lowest rolls. A value of 0 means to keep all rolls.
		 */
		public int	m_keep;

		/**
		 * Number of die rolls for this term. If negative, then the roll result is negated. If a bonus, than this is just
		 * the bonus value.
		 */
		public int	m_qty;
	...
	public String generateOutputString(final String rollerName, final List<Result> result, final String rollName)
	{
		...
		if (rollerName == null)
			ret.append("You Privately Roll ");
		else
			ret.append(UtilityFunctions.emitUserLink(rollerName) + " rolls ");
	 ...
	 */
	/*
	 * private String getMacroString() { if (!isInitialized()) { return ""; }
	 * 
	 * final StringBuffer buffer = new StringBuffer(); boolean bIsFirst = true;
	 * 
	 * for (int i = 0; i < m_terms.size(); i++) { final String dice = m_terms.get(i).toString(); if (!bIsFirst) {
	 * buffer.append(' '); }
	 * 
	 * final boolean negate = (dice.charAt(0) == '-'); if (negate) { buffer.append("- ");
	 * buffer.append(dice.substring(1).trim()); } else { if (!bIsFirst) { buffer.append("+ "); }
	 * 
	 * buffer.append(dice); } bIsFirst = false; }
	 * 
	 * return buffer.toString(); }
	 */
	 ...
	 */
	/*
	 * private String getRollString() { if (!isInitialized()) { return ""; }
	 * 
	...
	public boolean init(final String macro, final String name, final String parent)
	{
		...
		try
		{
			...
			String tokens[] = getMacro().split(";");
			for (String thisTerm : tokens)
			{
	...
	public List<Result> roll()
	{
		...
		for (int t = 0; t < m_terms.size(); t++)
		{
			...
			if (cur_term.m_repeat > 1)
			{
				...
				UtilityFunctions.arrayReverse(totals);
			}
